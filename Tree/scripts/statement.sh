...
pandoc -M contestName="${CONTEST_NAME}" -M problemLabel="${PROBLEM_LABEL}" -M problemName="${PROBLEM_NAME}" -M problemId="${PROBLEM_ID}" -T "" ${STATEMENT_DIR}/problem.md -o ${STATEMENT_DIR}/problem.pdf --template ${BASE_DIR}/../template.tex -f markdown -t latex -s --pdf-engine=xelatex
